type StateSetterFunc = React.Dispatch<
  React.SetStateAction<Record<string, never>>
>;
...
const register = (stateSetter: StateSetterFunc) =>
  stateSetters.push(stateSetter);

// Every Route component unregisters its state setter on unmount
const unregister = (stateSetter: StateSetterFunc) =>
  stateSetters.splice(stateSetters.indexOf(stateSetter), 1);
